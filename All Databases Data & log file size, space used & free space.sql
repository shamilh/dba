...
This SQL script will helpful for daily checklist, incase of no any third party tool is available for database monitoring.

 script will produce the all database size, status, recovery model, data & log file size, spaceused & free space details.

This script will work from sql server version 2005 & onwards. it will not work on SQL server 2000 as sys schema is not available in old version of SQL

In this script database those are offline will provide only status & recovery model value.

Note:- I have put limited characterset size for Dbname column, instead of varchar you can put sysname also. Just replace varchar to sysname as per your requirement. 
I have updated in below code section.
*/
